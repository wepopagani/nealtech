            ...
            {images.map((img, idx) => (
              <button
                ...
                className={`relative w-20 h-14 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${
                  idx === currentIndex ? 'border-[#38bdf8]' : 'border-transparent opacity-60 hover:opacity-100'
                }`}
              >
...
function ContactForm({ apartmentTitle }: { apartmentTitle: string }) {
  const [formData, setFormData] = useState({
    ...
    email: '',
    telefono: '',
    messaggio: `Sono interessato all'${apartmentTitle}. Vorrei ricevere maggiori informazioni e, se possibile, organizzare una visita.`
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
        ...
        className="w-full bg-gradient-to-r from-[#0d2644] via-[#1e6bb8] to-[#38bdf8] text-white py-4 rounded-lg font-semibold hover:shadow-xl hover:scale-[1.02] transition-all duration-300 disabled:opacity-70"
      >
    ...
    return (
      <div className="pt-20 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#0d2644]">Appartamento non trovato</h1>
          <Link href="/immobili/chiasso" className="mt-4 inline-block text-[#1e6bb8] hover:underline">
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex items-center space-x-2 text-sm">
            ...
            <Link href="/immobili/chiasso" className="text-[#64748b] hover:text-[#1e6bb8] transition-colors">
              Appartamenti Chiasso
            </Link>
            <svg className="w-4 h-4 text-[#64748b]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-lg">
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                  <span className="opacity-0 group-hover:opacity-100 bg-white/90 text-[#0d2644] px-4 py-2 rounded-full font-medium transition-opacity">
              ...
              <div className="p-4 grid grid-cols-4 md:grid-cols-6 gap-2">
                {apartment.images.slice(0, 6).map((img, idx) => (
                  ...
                  >
                    <Image src={img} alt="" fill className="object-cover" />
                    {idx === 5 && apartment.images.length > 6 && (
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="prose prose-gray max-w-none">
                {apartment.longDescription.split('\n\n').map((paragraph, idx) => (
                  <p key={idx} className="text-[#64748b] mb-4 leading-relaxed">
                    {paragraph}
                  </p>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {apartment.features.map((feature, idx) => (
                  <div key={idx} className="flex items-center">
                    <svg className="w-5 h-5 text-[#38bdf8] mr-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="font-semibold text-[#0d2644] mb-3 flex items-center">
                  ...
                  <ul className="space-y-2">
                    {apartment.included.map((item, idx) => (
                      <li key={idx} className="text-[#64748b] flex items-center">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {apartment.notIncluded.map((item, idx) => (
                      <li key={idx} className="text-[#64748b] flex items-center">
